...
# install nomachine
RUN curl -fSL "https://download.nomachine.com/download/7.7/Linux/nomachine_7.7.4_1_x86_64.rpm" -o nomachine.rpm && \
    rpm -i nomachine.rpm && \
    rm -rf nomachine.rpm

# add a user to access our desktop env
RUN groupadd -r nomachine -g 10001 && \
    useradd user -u 10000 -g nomachine -d /home/nomachine -m -s /bin/bash && \
